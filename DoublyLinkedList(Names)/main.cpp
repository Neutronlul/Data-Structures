...
// traverses through the list and returns the node that the new node should be inserted before
Node* findInsertPosition (NodeList &list, const std::string &data) {
    // start at the node after the head because insertNode() already checked the head
...
Node* findMatchingNode (NodeList &list, const std::string &data) {
    ...
    // traverse through the list until a matching name is found or the end is reached
...
void deleteNode (NodeList &list, const std::string &data) {
    ...
    if (deletionTarget) {
        ...
        if (deletionTarget == list.head) {
            ...
            // case 2: if the list is not empty at this point, set the new head's prev pointer to nullptr
            if (list.head) {
        ...
        if (deletionTarget == list.tail) {
            ...
            // case 3: if the list is not empty at this point, set the new tail's next pointer to nullptr
            if (list.tail) {
...
void displayDescending (const NodeList &list, std::ofstream &outputFile) {
    ...
    // for each node, print the name to the file and advance (retreat?) to the previous node
    while (curr) {
        outputFile << curr->name << std::endl;
        curr = curr->prev;
    }
